...
<img src="store/icon-144.png" width="72" align="right" alt="PebbleDoist icon">
...
- **Touch** (Pebble Time 2, firmware 4.33.2 or newer) — tap a row to open it or tick it off,
  drag a list to scroll it, swipe left-to-right to go back (from the overview that leaves the
  app, as BACK does), and tap a subtask in the detail view to tick it off. One tap is enough
  everywhere in the app. The buttons keep working exactly as before; after a touch-scroll the
  selection sits on the row in the middle of the screen, so the next button press carries on
  from what you are looking at. Menus the watch itself draws — a task's **Complete / Details /
  Add subtask / Delete**, for one — take two taps: the first highlights an entry, the second
  confirms it. A long press of **Select** opens those menus from anywhere.
...
**Touch in the emulator.** The pebble tool has no touch command, but qemu-pebble does emulate
the Time 2 touchscreen, so gestures can be injected over QEMU's QMP socket:

```sh
export PEBBLE_QEMU_PATH=$PWD/tools/qemu-touch-wrapper.sh   # adds -icount and the QMP socket
pebble install --emulator emery
tools/touch.py tap 100 60                  # screen pixels
tools/touch.py swipe 100 200 100 90
```

Needs **SDK 4.33.1 or newer**: the 4.33 emulator image still faults the moment an app opts
into touch navigation. The wrapper's `-icount` is a separate workaround, for emulated timers
stalling under WSL2.
...
- `src/c/` — `data` (fixed arrays + the on-demand detail buffer), `config` (AppMessage
  ...
  `touch_nav` (the app's own tap/pan recognizers on top of the system touch bridge, plus the
  hand-rolled raw-touch path kept behind `TOUCH_NAV_SYSTEM_BRIDGE`), `pebbledoist` (main;
  Quick-Launch gating via `launch_reason()`).

Store assets live in `store/`: `DESCRIPTION.md` (the listing text), `icon-80.png`
and `icon-144.png`, and `screenshots/`. The launcher menu icon and the store icons
are generated by `tools/gen_icon.py` / `tools/gen_marketing.py`
